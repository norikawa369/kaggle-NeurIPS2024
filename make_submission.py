...
def make_submission(args: TrainArgs):
    ...
    with Pool(processes=36) as pool:
        test_graphs = list(tqdm(pool.imap(smile_to_graph, df.molecule_smiles.values), total=len(df.molecule_smiles.values)))
    end = time.time()
    elapsed_time = end-start
    debug(f'{(elapsed_time // 3600):.0f}h{((elapsed_time % 3600) // 60):.0f}m{(elapsed_time % 3600 % 60):.0f}s to convert smiles->graphs.')

    test_smiles_graphs = dict(zip(df.molecule_smiles.values, test_graphs))
    
    debug('saving the made graphs!')
    start = time.time()
    with open(os.path.join(args.save_dir, 'test_smiles_graphs.pkl'), mode='wb') as f:
        pickle.dump(test_smiles_graphs,f)
    ...
    with torch.no_grad():
        ...
        for i, index in enumerate(np.arange(0,len(test_data.indexes), args.batch_size)):
        # for i, (indexes, binds) in enumerate(dataloader):
            
            indexes = test_data.indexes[index:index+args.batch_size]
            B = len(indexes)
            batch = dotdict(
                graph = my_collate(test_graphs, indexes, device=args.device),
            )



        # test_idx = np.arange(len(test_graphs))
        # for t, index in enumerate(np.arange(0,len(test_idx),args.batch_size)):
            
        #     index = test_idx[index:index+args.batch_size]
        #     batch = dotdict(
        #         graph = my_collate(test_graphs,index,device=args.device),
        #     )
